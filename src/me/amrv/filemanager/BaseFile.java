...
abstract class BaseFile {
	...
	/**
	 * Creates the remove queue for a better deleting system that allows to put in
	 * queue removal operations and cancell them before they are removed
	 */
	...
	/**
	 * Obtains the cannonical path from a file with the given directions, cannonical
	 * paths will format the path operators like <b>../</b> or <b>./</b>
	 *
	 * <p>
	 * Its planned to add the capability of this method to obtain the real path of
	 * link files
	 *
	 *
	 * @param parent      the abstract or full path of the file that will be used on
	 *                    conversion
	 * @param amount      the amount of subfolders the method will format before
	 *                    ending, use <b>-1</b> to canonicalize the full path, the
	 *                    amount of sections to handle starts from the end of the
	 *                    path
	 * @param addFileName if the cannonical path should include the file name or
	 *                    start with the first section of the path
	...
	/**
	 * Obtains the raw <a>java.io.File</a> that is being managed.
	 * 
	 * @return the raw file
	 */
	public File getFile() {
	 ...
	 * <tt>getAbstractPath()</tt> or <tt>getCannonicalPath()</tt> and if you want
	 * just the path to reach the file without the file name use
	 ...
	 * always needed or use <tt>getPath()</tt> to obtain the path that was used to
	 * define the file.
	 * <p>
	 * If you want to get the path without the file name use <tt>getParent()</tt> or
	 * <tt>getLastParent()</tt>
	 *
	...
	/**
	 * Gets a section of the cannonical path of the file from the disk, in example,
	 * a file with the path <b>temp/folder/data/file</b> and a section of <tt>2</tt>
	 * will return <b>folder/data/file</b>
	 *
	 * <p>
	 ...
	 * the file from the current location but it will contain only the path not the
	 * file itself. If the file was declared without any path this will return an
	 * empty string as it wont recognize any folder needed to reach the file
	 *
	...
	/**
	 * Works as <tt>getAbsolutePath()</tt> but it will return the path from the disk
	 ...
	 * example, a file with the path <b>temp/folder/data/file</b> and a section of
	 * <tt>2</tt> will return <b>folder/data</b>
	 *
	 * <p>
	 * If you want to get a section of the path including the file name in it, use
	 * <tt>getPathSection()</tt>
	 *
	 * @param section the amount of sub folders to obtain
	 ...
	 * greater than <tt>Long.MAX_VALUE</tt> then that value will be returned
	 *
	...
	/**
	 * Obtains the file of the size in bytes of the file or <tt>0L</tt> if the size
	 * cant be measured
	 *
	 * @return the size in bytes of the file or <tt>0L</tt> if it cant be measured
	 */
	 ...
	 * operations
	 *
	 * @param writable  if the file should be able to do write operations or not
	 * @param ownerOnly if this operation should be applied to the current
	 *                  user/owner or to everybody
	 */
	...
	/**
	 * Deletes the file or directory (directories must be empty for the operation to
	 * be acomplished).
	 * <p>
	 ...
	 * debugging purposes, however if the operation cant just be acoplished, it will
	 * just return <tt>false</tt>.
	 *
	...
	/**
	 * Forces the deletion of the requested file or folder, folders will be removed
	 * even if they are not empty and the operation cant be undone.
	 *
	 * <p>
	 * If you want the operation to not remove directories which are not empty, you
	 * can use <tt>delete()</tt>.
	 *
	 ...
	 * cancel it, except from that this work like <tt>delete()</tt>.
	 * <p>
	 ...
	 * <tt>deleteOnExit()</tt> except this allows to cancel the operation before the
	 * file gets removed
	 *
	 ...
	 *
	 * <p>
	 * Runtime exceptions may be thrown if a severe problem happens, however if the
	 * operation just fails it will just return a false value
	 *
	 ...
	 * contents but keeping all the data that the virtual machine holds, allowing
	 * for more operations after saving.
	 *
	 * <p>
	 * Having multiple applications with the same file opened may cause problems
	 * depending on how do they use streams to save files, also runtime exceptions
	 * may be thrown for debugging purposes.
	 *
	...
	/**
	 * This will return the size in bytes of the file, not denoted by itself but by
	 * the abstract pathname, some files may return 0L if there was a problem
	 ...
	 * may be thrown for debugging purposes.
	 *
	 * @param mode if the save operation should use a modern method, sometimes
	 *             faster on newer computers due to the use of buffers
	 * @return true if the operation was succesful, false otherwise
	 */
	...
	/**
	 * Removes all the data stored on the virtual machine so a save operation would
	 * remove all the contents from the file.
	 */
	abstract public void clear();

	/**
	 * Protected method to read the file information, this method gets executed once
	 * all the checks of aviability are done.
	 *
	...
	/**
	 * Protected method to save the file information, this method gets executed once
	 * all the checks of aviability are done.
	 *
	 * <p>
	...
	/**
	 * Protected method to save the file information, this method gets executed once
	 * all the checks for aviability are done.
	 *
	 * <p>
	...
	/**
	 * Protected method to save the file information, this method gets executed once
	 * all the checks for aviability are done but wont rewrite the file contents,
	 * instead it will add whatever was stored on the virtual machine at the end of
